...
function initMap(origen, destino, peso) {
    ...
    service.getDistanceMatrix(request)
        .then((response) => {
            ...
            guardarTotalStorage(totalFlete);
            // Invocamos funciones para mostrar la simulacion al usuario, y guardar en el storage local
            pintarFlete(simulacionFlete, totalFlete);
            guardarSimulacionStorage(simulacionFlete);
...
// Funcion para pintar datos del flete

function pintarFlete(simulacionFlete, totalFlete) {
...
Toastify({
    text: 'Flete simulado correctamente!',
    duration: 3000,
    style: {
        background: '#25282A'
    }
...
const guardarSimulacionStorage = (simulacionFlete) => {
    localStorage.setItem('simulacionFlete', JSON.stringify(simulacionFlete));
};

const obtenerSimulacionStorage = () => {
    const simulacionStorage = JSON.parse(localStorage.getItem('simulacionFlete'));
    return simulacionStorage;
};

const guardarTotalStorage = (totalFlete) => {
    localStorage.setItem('totalFlete', JSON.stringify(totalFlete));
};

const obtenerTotalStorage = () => {
    const totalStorage = JSON.parse(localStorage.getItem('totalFlete'));
    return totalStorage;
};
